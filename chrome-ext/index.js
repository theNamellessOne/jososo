...
button.addEventListener("click", async () => {
  try {
    ...
    const jobsenderbotCookies = await chrome.cookies.getAll({
      url: "http://localhost:3000",
    });

    const liap = linkedInCookies.find((cookie) => cookie.name === "liap");
    const authCookies = jobsenderbotCookies.find(
      (cookie) => cookie.name === "AuthToken",
    );

    if (!liap || liap.expirationDate * 1000 < Date.now()) {
      text.innerText = "Please login to Linkedin";
      return;
    }

    if (!authCookies || authCookies.expirationDate * 1000 < Date.now()) {
      text.innerText = "Please login to Jobsenderbot";
      return;
    }

    const authToken = jobsenderbotCookies.find(
      (cookie) => cookie.name === "AuthToken",
    ).value;

    const response = await fetch("http://localhost:3000/api/linkedin/login", {
      method: "POST",
